...
void ExchangeDnaFragments(struct uint16_genome * genome1, struct uint16_genome * genome2) {
    ...
    int fragmentsToExchange1 = RangedRandomNumber(0, genome1->size - pos1 - 1);
    int fragmentsToExchange2 = RangedRandomNumber(0, genome2->size - pos2 - 1);
...
void RandomFragmentCopy(struct uint16_genome * genome) {
    ...
    int fragmentsToCopy = RangedRandomNumber(0, genome->size - pos - 1);
...
void RandomFragmentMove(struct uint16_genome * genome) {
    int pos = RangedRandomNumber(0, genome->size - 1);
    int fragmentsToMove = RangedRandomNumber(0, genome->size - pos - 1);
    ...
    int newPos = RangedRandomNumber(0, newSize - 1)/2;
...
void RandomFragmentDuplicate (struct uint16_genome * genome) {
    int pos = RangedRandomNumber(0, genome->size - 1);
    int fragmentsToDuplicate = RangedRandomNumber(0, genome->size - pos - 1);
...
void RandomFragmentInsetrion(struct uint16_genome * genome) {
    ...
    int fragmentsToInsert = RangedRandomNumber(0, MAX_FRAGMENTS_TO_INSERT);
...
void RandomFragmentDeletion(struct uint16_genome * genome) {
    ...
    int fragmentsToRemove = RangedRandomNumber(0, genome->size - remove - 1);
