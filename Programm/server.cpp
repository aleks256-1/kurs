...
{  
    ...
    ssize_t rec;
    int count = 0, res;
    int creature_data[4][128];
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < 128; i++) {
            creature_data[j][i] = RangedRandomNumber(0, 255);
        }
    }

    int startClients;
    cout << "How many clients should connect before the start?\n";
    cin >> startClients;
    startClients--;
    while(TRUE)  
    {  
        ...
        do {
            ...
            {  
                ...
                for (int j = 0; j < 4; j++) {
                    for (int i = 0; i < 128; i++) {
                        strcpy(buffer, to_string(creature_data[j][i]).c_str());
                        rec = 0;
                        do {                        
        ...
        } while (client_socket[startClients] == 0);
        ...
        int numberOfMutations = RangedRandomNumber(NEXT_GENERATION_POPULATION/2, 10*NEXT_GENERATION_POPULATION); 
        for (int i = 0; i < numberOfMutations; i++) {                                           
            int mutation = RangedRandomNumber(0,100);                                          
            int creature = RangedRandomNumber(START_POPULATION, NEXT_GENERATION_POPULATION - 1);                        
            if (mutation < 13) {                     
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 128; i++) { 
                    imageCreature->cells[j].v[i] = imageCreature->cells[j].dv[i] = creature_data[j][i];
            }
        }
        int step = 0;
        ...
        if (imageGenome) {
            free(imageGenome->genes);
            free(imageGenome);
        }
        if (imageCreature) {
            free(imageCreature->cells);
            free(imageCreature);
        }
