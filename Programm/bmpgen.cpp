...
void createImage(struct creature * creature, char* imageFileName) {
    ...
    for(int i = 0; i < creature->n; i++){
        for(int j = 0; j < creature->n; j++){
            ...
            for (int h = 0; h < 10; h++) {
                red += creature->cells[j*creature->n+j].v[h];
                green += creature->cells[j*creature->n+j].v[h+10];
                blue += creature->cells[j*creature->n+j].v[h+20];
            }
            image[i][j][2] = (unsigned char)(red/10); ///red
            image[i][j][1] = (unsigned char)(green/10); ///green
            image[i][j][0] = (unsigned char)(blue/10); ///blue
        }
    }
