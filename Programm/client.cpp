...
int main(int argc, char ** argv) {
 ...
 // recive the welcome message from server
    ...
    image = readBMP((char*)"rainbow.bmp");
    ...
    do {
        ...
        if (atoi(buffer) == 123456789) {
            ...
            if (nextGeneration->size == 0) {
                ...
                strcpy(buffer, to_string(1001.0).c_str());
                rec = 0;
                do {
                    int res = send(client, &buffer[rec], bufsize-rec, 0);
                    rec+=res;
                } while (rec<bufsize);
                cout << "Creature # " << genomeNumber << " " << 1001 << '\n';
            } else {
                ...
                nextGeneration->similarity = 65536;      
